...
{
    oops_rtos_task_list *task_prt_new = \
        (oops_rtos_task_list*)oops_rtos_malloc(sizeof(oops_rtos_task_list));
    ...
    if(NULL == task_list){
        ...
        for (;del_node != (*task_list)->NextNode;del_node = (*task_list)->NextNode){
            if (((del_node->TaskPtr) == p_tcb) && \
                (del_node->TaskPtr->Priority != IDLE_TASK_PRIORITY)){
 ...
 * @date 2019-11-25
 */
oops_rtos_task_list* OOPS_RTOS_SearchNodeTaskList_Prev(oops_rtos_tcb *p_tcb,\
 ...
 * @date 2019-11-25
 */
void oops_rtos_task_move(oops_rtos_tcb *target_tcb,
                         oops_rtos_task_list **src_list,
                         oops_rtos_task_list **tar_list)
{
...
oops_rtos_tcb*  oopsrtoe_get_highest_rdy_task(void)
{
    ...
    if ((OOPS_RTOS_RdyTaskList->TaskPtr->Priority <= ret_tcb->Priority) || \
        (OOPS_RTOS_RdyTaskList->TaskPtr->Virtual_Pri <= ret_tcb->Virtual_Pri))
        ret_tcb =  OOPS_RTOS_RdyTaskList->TaskPtr;

    for (; OOPS_RTOS_RdyTaskList != bak_node; \
         OOPS_RTOS_RdyTaskList = OOPS_RTOS_RdyTaskList->NextNode){
            if ((OOPS_RTOS_RdyTaskList->TaskPtr->Priority <= ret_tcb->Priority) || \
                (OOPS_RTOS_RdyTaskList->TaskPtr->Virtual_Pri <= ret_tcb->Virtual_Pri))
                ret_tcb =  OOPS_RTOS_RdyTaskList->TaskPtr;
    }
